...
exports.generateReport = exports.analyzeResponses = void 0;
 ...
 */
exports.analyzeResponses = functions
    ...
    try {
        ...
        if (!projectDoc.exists) {
            throw new functions.https.HttpsError("not-found", "프로젝트를 찾을 수 없습니다.");
        }
        // 응답 데이터 조회
        ...
        if (responsesSnapshot.empty) {
            throw new functions.https.HttpsError("not-found", "분석할 응답이 없습니다.");
        }
        // 응답 데이터 정리
...
`;
        ...
        throw new functions.https.HttpsError("internal", `분석 중 오류가 발생했습니다: ${error}`);
    }
});
/**
...
${questionStats
            .slice(0, 20)
            .map((q) => `- ${q.question}: 응답 ${q.responseCount}개, 평균 ${q.averageRating || "N/A"}점`)
            .join("\n")}
...
`;
        ...
        throw new functions.https.HttpsError("internal", `보고서 생성 중 오류가 발생했습니다: ${error}`);
    }
});
